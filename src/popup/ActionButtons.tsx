...
const ActionButtons: FC<Props> = (props) => {
  ...
  return (
    <div className="flex items-center justify-end col-span-1 gap-2">
      ...
      <button className="relative flex justify-center p-1 rounded-full group bg-slate-600" onClick={deleteSnip}>
        ...
        <span className="absolute z-50 w-auto p-1 text-xs capitalize transition-all duration-100 origin-bottom scale-0 bg-white rounded-md shadow-md dark:bg-slate-900 dark:text-slate-100 text-slate-800 tooltip min-w-max bottom-7 group-hover:delay-300 group-hover:scale-100">Delete Snip</span>
      </button>
    </div>
  );
};
