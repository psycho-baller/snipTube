...
export function exportSnip(snip: Snip) {
  ...
  navigator.clipboard.writeText(
    ...
    #### ${title} | (${Math.floor(startTimestamp / 60)}:${String(Math.round(startTimestamp) % 60).padStart(
      2,
      "0"
    )} - ${Math.floor(endTimestamp / 60)}:${String(Math.round(endTimestamp) % 60).padStart(2, "0")})
